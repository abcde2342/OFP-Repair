...
CASE_TITLE = 'BUG: arccos inaccurate for small complex numbers with mingw-w64'

CASE_FUNCTION = 'numpy.arccos(complex)'

CASE_ID = 'numpy/numpy#6081'
...
def arccos_taylor(z):
    return math.pi / 2 - z - z ** 3 / 6 - 3 * z ** 5 / 40
...
def print_case_report() -> None:
    ...
    expressions = (
        'np.arccos(z)',
        'mp.acos(mp.mpc(z.real, z.imag))',
        'arccos_taylor(z)',
    )

    oracle_value = mp.acos(mp.mpc(z.real, z.imag)).imag
    original_value = -9.9920072216263584e-15j.imag
    taylor_value = arccos_taylor(z).imag

    original_relative_error = relerr(original_value, oracle_value)
    taylor_relative_error = relerr(taylor_value, oracle_value)
    condition, _ = _condition_from_finite_difference(mp.acos, mp.mpc(z.real, z.imag))
