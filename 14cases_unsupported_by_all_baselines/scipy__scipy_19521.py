...
CASE_TITLE = 'ENH: `scipy.special.log_softmax` could be `2**126` to `2**1022` times more accurate'

CASE_FUNCTION = 'scipy.special.log_softmax'

CASE_ID = 'scipy/scipy#19521'
...
def case_scipy_scipy_19521() -> dict[str, Any]:
    return _log_softmax_dominant_case('scipy/scipy#19521', ('x_i - max(x) - log(sum(exp(x-max(x))))', 'mpmath shifted logsumexp', 'x_i-max(x)-(s - s**2/2 + s**3/3 - ...), s=sum(exp(nonmax))'))

def print_case_report() -> None:
    rows, patch_idea = case_scipy_scipy_19521()
