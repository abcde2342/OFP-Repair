...
CASE_TITLE = 'BUG: spatial: premature overflow in minkowski distance'

CASE_FUNCTION = 'scipy.spatial.distance.minkowski / numpy.linalg.norm'

CASE_ID = 'scipy/scipy#21114'
...
def pnorm_naive(vals, p):
    ...
    X = arr
    Y = np.zeros_like(X)
    return float(minkowski(X, Y, p=p))
...
def case_scipy_scipy_21114() -> dict[str, Any]:
    case = 'scipy/scipy#21114'
    expressions = ('(sum(abs(x_i)**p))**(1/p)', 'mpmath p-norm', 'max(abs(x))*(1+s)**(1/p) binomial Taylor')
    vals = [10.0, 20.0, 3.3, 7.3, 8.8]
    p = 1000000.0
    ...
    condition = _condition_from_vector_finite_difference(lambda xs: _mp_pnorm(xs, mp.mpf('100000')), vals)
...
def print_case_report() -> None:
    rows, patch_idea = case_scipy_scipy_21114()
