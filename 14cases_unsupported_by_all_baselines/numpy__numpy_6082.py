...
CASE_TITLE = 'BUG: arcsin inaccurate for small complex numbers'

CASE_FUNCTION = 'numpy.arcsin(complex)'

CASE_ID = 'numpy/numpy#6082'
...
def print_case_report() -> None:
    ...
    expressions = (
        'np.arcsin(z)',
        'mp.asin(mp.mpc(z.real, z.imag))',
        'arcsin_taylor(z)',
    )

    oracle_value = mp.asin(mp.mpc(z.real, z.imag)).imag
    original_value = (0.010000166674167112+1.0103029524088975e-14j).imag
    taylor_value = arcsin_taylor(z).imag

    original_relative_error = relerr(original_value, oracle_value)
    taylor_relative_error = relerr(taylor_value, oracle_value)
    condition, _ = _condition_from_finite_difference(mp.asin, mp.mpc(z.real, z.imag))
